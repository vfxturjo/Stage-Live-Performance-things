...
; app related menu
ctxmenu_App := Menu()
ctxmenu_App.Add("Reload", (*) => (Reload()))
ctxmenu_App.Add("Exit", (*) => (ExitApp))
ctxmenu_QuickLoad := Menu()
; quickLoader
ctxmenu_QuickLoad.Add("ReloadFiles - !!!TODO", (*) => (0))
ctxmenu_QuickLoad.Add()
ctxmenu_QuickLoad.Add("File1 in keyBindingsFolder", (*) => (0))
ctxmenu_QuickLoad.Add("File2 in keyBindingsFolder", (*) => (0))
; main context menu
ctxmenu_ContextMenu := Menu()
ctxmenu_ContextMenu.Add("KeyBindings", (*) => (Run("SettingsGUI.ahk")))
ctxmenu_ContextMenu.Add("TODO - QuickLoad", ctxmenu_QuickLoad)
ctxmenu_ContextMenu.Add()
ctxmenu_ContextMenu.Add("App", ctxmenu_App)
window.OnEvent("ContextMenu", (*) => (ctxmenu_ContextMenu.Show()))
;
...
; Dictionary to store the midi functions and their respective handlers
MidiFunctionHandlers := {
    ; pattern
    ; name:[function_onPress, function_onRelease, (optional action MODIFIER)]
    note: [PressMidiNote, ReleaseMidiNote, GetNoteID],
    bend: [bendPitchPress, bendPitchRelease],
    sust: [SustainPedalPress, SustainPedalRelease],
    trans: [transposeKeyPress, transposeKeyRelease],
    chan: [ChannelKeyPress, ChannelKeyRelease],
    velo: [VelocityKeyPress, VelocityKeyRelease],
    mute: [MuteKeyPress, MuteKeyRelease]
}

; Function to register a hotkey for midi function
RegisterMidiHotkey(SCcodeID, functions, action) {
    try {
        ; optional action modifier
        if functions.Length > 2 { ; only for Midi note.. convert to id
            action := functions[3](action)
        }

        ; main hotkey binding
        HotIf (wnd := 'ahk_class ' WinGetClass(), (*) => WinActive(wnd))
        hotkey("~*SC" SCcodeID, functions[1].bind(A_Index, action))

        ; if only onRelease is needed
        if functions.Length > 1 {
            hotkey("~*SC" SCcodeID ' up', functions[2].bind(A_Index, action))
        }
    }
    catch {
        MsgBox "Problem with " action ", key:" SCcodeID
    }
}

; #region reading keyBinding files
for entry, vals in kbdUI_AllKBDinfo {
        ...
        if (IsItemInList(thisKeySetting[1], AvailableFunctions)) {
            ...
            if (thisKeySetting[1] == "app") {
                if (IsItemInList(thisKeySetting[2], AvailableAppFunctions)) {
                    ...
                    ToolTip("Problem with app hotkeys! SC:" SCcodeID)
                }

            } else if (thisKeySetting[1] == "midi") {
                if (IsItemInList(thisKeySetting[2], AvailableMidiFunctions)) {
                    functions_for_this_key := MidiFunctionHandlers.%thisKeySetting[2]%
                    params_for_this_key := thisKeySetting[3]
                    RegisterMidiHotkey(SCcodeID, functions_for_this_key, params_for_this_key)
                } else {
                    MsgBox "Data not available in function. Must be a note OR ERROR, key: " vals[iniKeyName]
                }
            }
        } else {
