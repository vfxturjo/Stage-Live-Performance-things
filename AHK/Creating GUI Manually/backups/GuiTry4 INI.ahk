...
Loop read, "csv/KeyboardButtonInfos.csv"
{
	LineNumber := A_Index
	if (LineNumber == 1)
		continue
...
CreateGUI() {
	...
	createKeyboardUI() {
		...
		for entry in kbdUI_AllKBDinfo {
			...
			kbdUI_keysMap[kbdUI_Keys_id] := myGui.Add("Button", "x" thisButtonX " y" thisButtonY " w" kbdUI_defaultButtonSize " h" kbdUI_defaultButtonSize, vals[KeyName])
